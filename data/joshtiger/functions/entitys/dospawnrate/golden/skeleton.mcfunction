...
tellraw @a[tag=Debug2] [{"score":{"name":"@e[tag=RandomzMain]","objective":"RN1"}},{"text":"x"},{"score":{"name":"@e[tag=RandomzMain]","objective":"RN2"}},{"text":"="},{"score":{"name":"@e[tag=RandomzMain]","objective":"RN"}}]
execute as @e[tag=RandomzMain,scores={RN=1..1}] at @s unless entity @e[type=skeleton,tag=Golden] run function joshtiger:entitys/spawninworld/golden/skeleton
